...
public class ConcatColumnsProcessingNodeTest {
    ...
    ConcatColumnsProcessingNode node = new ConcatColumnsProcessingNode(
            "testId",
            "processing",
            "concat_column",
            "input1",
    ...
    @Test
    public void shouldReturnResultOfConcatColumnsProcessingIfCorrectNumberOfDataElementsIsCorrect() {
        when(csvData.size()).thenReturn(1);

        try (
                MockedStatic<ConcatColumnsProcessor> concatColumnsProcessorMockedStatic = Mockito.mockStatic(ConcatColumnsProcessor.class);
            ...
            concatColumnsProcessorMockedStatic
                    .when(() -> ConcatColumnsProcessor.concat(input, "test.input1", "test.input2", "test"))
                    .thenReturn(csv);
